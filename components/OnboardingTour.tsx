...
import { AccessibilityInfo, BackHandler, Platform, ScrollView, Text, TouchableOpacity, StyleSheet, useWindowDimensions } from "react-native";
...
export default function OnboardingTour({ visible, originRoute, onFinish }: Props) {
  ...
  useEffect(() => {
    ...
    async function ensureVisible(rect: TourTargetRect): Promise<TourTargetRect> {
      ...
      const containerRect = await measureScrollRoot(activeStep.scrollRootId);
      ...
      for (let attempt = 0; attempt < MAX_SCROLL_SETTLE_ATTEMPTS; attempt++) {
        ...
        const measured = await measureTarget(activeStep.targetId);
        if (!measured) break;
    ...
    async function locateTarget() {
      ...
      for (let attempt = 0; attempt < MAX_MEASURE_ATTEMPTS; attempt++) {
        if (cancelled) return;
        let rect = await measureTarget(activeStep.targetId);
        if (rect) {
          if (cancelled) return;
